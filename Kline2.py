...
class TradingSimulator:
    ...
    def _start_simulation(self):
        ...
        if not market:
            messagebox.showwarning("提示", "请先选择市场类型")
            return
            
        try:
            # 检查数据路径
            if not os.path.exists(self.stock_pool[market]):
                raise FileNotFoundError(f"{market}数据路径不存在")
                
            # 随机选择股票文件
            valid_files = [f for f in os.listdir(self.stock_pool[market]) 
                          if f.endswith('.day') and f.startswith(('sh', 'sz'))]
            if not valid_files:
                raise ValueError("该市场没有有效数据文件")
                
            stock_file = random.choice(valid_files)
            file_path = os.path.join(self.stock_pool[market], stock_file)
            
            # 加载数据
            self.current_data = read_day_file(file_path)
            required_days = SIMULATION_DAYS + 65
            if len(self.current_data) < required_days:
                raise ValueError(f"至少需要 {required_days} 天的历史数据")
    ...
    def _update_chart(self):
        ...
        mpf.plot(
            ...
            ax=self.ax_price,
            returnfig=True,
            warn_too_much_data=len(plot_data)+1
        )
